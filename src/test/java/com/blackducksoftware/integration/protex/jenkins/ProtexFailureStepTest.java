...
public class ProtexFailureStepTest {
    ...
    private ProtexFacade getMockedFacade(final int pendingIds, final int licenseViolations) throws Exception {
        ProtexFacade mockedFacade = Mockito.mock(ProtexFacade.class);
    ...
    private AbstractBuild getMockedAbstractBuild(Result buildResult, PostBuildProtexScan protexScan, boolean scanRun, ProtexFailureStep failureStep)
            throws Exception {
        ...
        Mockito.doAnswer(new Answer<FreeStyleProject>() {
            @Override
            public FreeStyleProject answer(InvocationOnMock invocation) {
                return proj;
            }
    ...
    public void testGetRequiredMonitorService() {
        ProtexFailureStep failureStep = new ProtexFailureStep(false, false);
    ...
    public void testProtexFailureStepCreationTrue() {
        ProtexFailureStep failureStep = new ProtexFailureStep(true, true);
    ...
    public void testCheckProtexFailConditionsPendingIdsWithoutFailureCondition() throws Exception {
        ProtexFailureStep failureStep = new ProtexFailureStep(false, true);
        TestBuildListener listener = new TestBuildListener(currStream);
    ...
    public void testCheckProtexFailConditionsLicenseViolationsWithoutFailureCondition() throws Exception {
        ProtexFailureStep failureStep = new ProtexFailureStep(true, false);
        TestBuildListener listener = new TestBuildListener(currStream);
        ProtexJenkinsLogger logger = new ProtexJenkinsLogger(listener);
        ...
        assertTrue(failureStep
                .perform(
                        getMockedAbstractBuild(Result.SUCCESS,
                                new PostBuildProtexScan(uuid, validCredentials, testProperties.getProperty("TEST_PROJECT_NAME"), "", "", 2.0, null), false,
                                failureStep),
                        null,
                        listener));
    ...
    public void testPerform() throws Exception {
        ...
        PostBuildProtexScan postScan = new PostBuildProtexScan(uuid, validCredentials, "FakeProjectName", "", "", 2.0, null);
        ProtexFailureStep failureStep = new ProtexFailureStep(true, true);
        ...
        orgStream.println(output);
        assertTrue(output.contains("Checking the scan results against the Failure Conditions"));
        assertTrue(output.contains("Finished running Protex Failure Conditions."));
    }
